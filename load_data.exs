defmodule DataLoader do
  alias CoffeeRoulette.{Group, Participant}
  ...
  def participant_from_rounds_row([email | [guild | [tod_pref | _rest]]] = rounds_row) do
    case Participant.new(email, String.to_atom(guild), tod_pref) do
    ...
    |> Enum.reduce(%{}, fn raw_participant_rounds, groups_by_round ->
      ...
      |> Enum.reduce(groups_by_round, fn {group_number_str, index}, groups ->
        with group_id = group_id_from_str(group_number_str, time_of_day),
             ...
             group_access_fn = Access.key(group_id, Group.new)
        do
          update_in(
  ...
  def merge_rounds_groups(rounds_groups) do
    rounds_groups |> Enum.map(&Map.values/1) |> Enum.zip |> Enum.map(&merge_round_groups/1)
  end
...
alias CoffeeRoulette.{Round, Group, Participant}

{:ok, raw_response_body} = File.read("/Users/britt/Downloads/gsheets-values-coffee-roulette-response.json")
...
# more useful representations of coffee roulette participants and their past
# coffee roulette groups
# keys: [:groups_by_round, :participants, :raw_form_values, raw_afternoon_values]
stuff = DataLoader.extract_overkill(raw_response_body)

# 0 Request sheet
# 1 Parse sheet
# 2 Hydrate data structures
# 3 Sort
